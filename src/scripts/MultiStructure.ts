...
=-=============-=
Copyright: Misode (https://github.com/misode/vscode-nbt/blob/master/src/editor/MultiStructure.ts)
MIT License 
  ...
  getBlock(pos: BlockPos): PlacedBlock | null {
    for (const region of this.regions) {
      if (MultiStructure.posInRegion(pos, region)) {
        const block = region.structure.getBlock(
          BlockPos.subtract(pos, region.pos)
        );
        if (block !== null) {
          return block;
        }
