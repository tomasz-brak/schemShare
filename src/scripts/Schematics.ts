...
=-=============-=
Copyright: Misode (https://github.com/misode/vscode-nbt/blob/master/src/editor/Schematics.ts)
MIT License 
...
export function litematicToStructure(root: NbtCompound) {
  const enclosingSize = root
    .getCompound("Metadata")
    .getCompound("EnclosingSize");
  ...
  root.getCompound("Regions").forEach((name, region) => {
    ...
    const palette = region
      .getList("BlockStatePalette", NbtType.Compound)
      .map(BlockState.fromNbt);
